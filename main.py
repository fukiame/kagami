...
def detect_dangerous_code(content):
    dangerous_patterns = [
        rb'\b(alias|eval|base64|rm -r|rm -fr|if=/dev/random|if=/dev/urandom|if=/dev/null|if=/dev/zero|cmd erase|apparmor|setenforce|shred -f|ufw disable|iptables -F|setfacl|/proc/sysrq-trigger|:(){ :|:& };:)\b',
    ]
